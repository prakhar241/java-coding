...
/**
 * The {@code BinarySearchTree} implementation. A BST is a binary tree in
 * symmetric order. A binary tree is either: empty or has two disjoint binary
 * trees (left or right). Symmetric order implies: each node has key and every
 * node's key is larger than all keys in left subtree and smaller than all keys
 * in right subtree.
 *
 * @author prakbans
...
public class BinarySearchTree<Key extends Comparable<Key>> implements BinaryTree<Key> {
    ...
    private boolean hasSum(Node node, double sum) {
        if (node == null) {
            return false;
        } else if (node.left == null && node.right == null) {
            return (sum == (Double) node.key);
        } else {
            return hasSum(node.left, sum - (Double) node.key) || hasSum(node.right, sum - (Double) node.key);
        }
    }
